...
class StoreProductsTableViewController: UIViewController {
            ...
            .getDocuments { [weak self] (querySnapshot, error) in
                ...
                self.products = querySnapshot?.documents.compactMap { doc -> Product? in
                    ...
                    return Product(
                        ...
                        price: data["price"] as? String ?? "0", // Default to "0" if price is nil
    ...
    @IBAction func selectItemTapped(_ sender: UIButton) {
        ...
        alertController.addAction(UIAlertAction(title: "Price: Highest to Lowest", style: .default, handler: { _ in
            self.filteredProducts.sort {
                (Double($0.price) ?? 0) > (Double($1.price) ?? 0)
            }
            self.tableView.reloadData() // Reload the table view to reflect changes
        }))
        
        alertController.addAction(UIAlertAction(title: "Price: Lowest to Highest", style: .default, handler: { _ in
            self.filteredProducts.sort {
                (Double($0.price) ?? 0) < (Double($1.price) ?? 0)
            }
            self.tableView.reloadData() // Reload the table view to reflect changes
...
extension StoreProductsTableViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Assuming you have a label for displaying the price
        cell.productPrice.text = product.price // Display the price in the cell
